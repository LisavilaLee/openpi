...
CUDA_VISIBLE_DEVICES=4,5,6,7 XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 uv run scripts/train.py pi05_libero --exp-name=reproduce_libero --resume  --batch-size=256 --fsdp-devices=4 --save-interval=1000
```
...
等到出现：`INFO:websockets.server:server listening on 0.0.0.0:8000`
